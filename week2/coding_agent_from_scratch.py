...
# Allow importing llm.py from the same directory
...
def edit_file_tool(path: str, old_str: str, new_str: str) -> Dict[str, Any]:
    ...
    creates/overwrite file with new_str.
    :param path: The path to the file to edit.
    :param old_str: The string to replace.
    :param new_str: The string to replace with.
    :return: A dictionar with the path to the file and the action taken
    """
    full_path = resolve_abs_path(path)
    if old_str == "":
...
def execute_llm_call(conversation: List[Dict[str, str]]):
    response = chat(model="kimi-for-coding", messages=conversation)
    return response.message.content

## Todo: 编写提示词完成这个coding agent， 参考 week2/psudo_code.md
